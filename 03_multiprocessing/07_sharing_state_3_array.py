...
import time
from multiprocessing import Array, Process, Queue, Semaphore
...
def task(val, sem1, sem2):
    ...
    logger.info(f"Received {[i for i in val]} (id {id(val)})")
    sem1.release()
    sem2.acquire()
    logger.info(f"After change: {[i for i in val]} (id {id(val)})")
